...
namespace NPoco.SqlServer
{
    public class SqlBulkCopyHelper
    {
        ...
        {
            return BulkInsertAsync(db, list, sqlBulkCopyOptions, CancellationToken.None);
        }

        public static Task BulkInsertAsync<T>(IDatabase db, IEnumerable<T> list, InsertBulkOptions sqlBulkCopyOptions, CancellationToken cancellationToken)
        {
            return BulkInsertAsync(db, list, SqlBulkCopyOptions.Default, sqlBulkCopyOptions, cancellationToken);
        }

        public static async Task BulkInsertAsync<T>(IDatabase db, IEnumerable<T> list,
            SqlBulkCopyOptions sqlBulkCopyOptions, InsertBulkOptions insertBulkOptions)
        {
            await BulkInsertAsync(db, list, sqlBulkCopyOptions, insertBulkOptions, CancellationToken.None);
        }

        public static async Task BulkInsertAsync<T>(IDatabase db, IEnumerable<T> list, SqlBulkCopyOptions sqlBulkCopyOptions, InsertBulkOptions insertBulkOptions, CancellationToken cancellationToken)
        {
            using (var bulkCopy = new SqlBulkCopy(SqlConnectionResolver(db.Connection), sqlBulkCopyOptions, SqlTransactionResolver(db.Transaction)))
            {
                var table = BuildBulkInsertDataTable(db, list, bulkCopy, sqlBulkCopyOptions, insertBulkOptions);

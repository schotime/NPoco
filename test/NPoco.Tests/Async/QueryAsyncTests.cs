...
namespace NPoco.Tests.Async
{
    ...
    public class QueryAsyncTests : BaseDBFluentTest
    {
        ...
        {
            var source = new CancellationTokenSource();
            var user = await Database.FirstOrDefaultAsync<User>("where userid = @0", 1, source.Token);
            Assert.AreEqual(1, user.UserId);
            var user1 = await Database.FirstAsync<User>("where userid = @0", 1, source.Token);
            Assert.AreEqual(1, user1.UserId);
        }
        ...
        public void QueryAsyncFirst_WithCancelledCancellationToken_ShouldThrow()
        {
            ...
            Assert.ThrowsAsync<TaskCanceledException>(() =>
                Database.FirstOrDefaultAsync<User>("where userid = @0", 1, source.Token));
            Assert.ThrowsAsync<TaskCanceledException>(() =>
